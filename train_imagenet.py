...
class Trainer:
    ...
    def create_dir(self):
        ex_path = os.path.join(cfg.imagenet['experiments_path'], str(self.ex_num), self.config['name'])
        os.makedirs(ex_path)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(cfg.imagenet['experiments_path']):
        os.makedirs(cfg.imagenet['experiments_path'])
    ex_num = len(os.listdir(cfg.imagenet['experiments_path']))

    for backbone in backbone_list:
        if backbone == 'ResNet18':
            model = ResNet18.ResNet18(class_num=cfg.imagenet['class_num'], pretrained=True)
            model.to(device)
            ...
            axs[0].set_title('Loss-ImageNet')
            axs[1].plot(epoch_auc_history, label=cfg.resnet18['name'])
            axs[1].set_title('Accuracy-ImageNet')
            axs[1].set_xlabel('Epoch')
            axs[1].set_ylabel('Accuracy')
            del model, train

        elif backbone == 'ResNet50':
            model = ResNet50.ResNet50(class_num=cfg.imagenet['class_num'], pretrained=True)
            model.to(device)
            ...
            model = ResNet101.ResNet101(class_num=cfg.imagenet['class_num'], pretrained=True)
            model.to(device)
            ...
            model = MobileNetV2.MobileNetV2(class_num=cfg.imagenet['class_num'], pretrained=True)
            model.to(device)
            ...
            model = EfficientNetV2.EfficientnetV2(class_num=cfg.imagenet['class_num'], pretrained=True)
            model.to(device)
            ...
            model = VGG19.Vgg19(class_num=cfg.imagenet['class_num'], pretrained=True)
            model.to(device)
            ...
            fig.savefig(os.path.join(cfg.imagenet['experiments_path'], str(ex_num), 'all.png'))
            del model, train